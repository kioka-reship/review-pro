...
const QUESTIONS = [
  ...
  { id: "highlight", label: "良かった点を教えてください", type: "multi", options: ["料理・味", "スタッフの接客", "お店の雰囲気", "価格・コスパ", "立地・アクセス"] },
  { id: "feel", label: "一言でいうと？", type: "select", options: ["また来たい！", "友人に勧めたい", "期待以上だった", "安心して頼める"] },
];
...
async function generateReview(store, answers, style) {
  ...
  return (
    <div onClick={!loading ? onSelect : undefined}
      style={{
        ...
        padding: "16px", cursor: loading ? "default" : "pointer",
      ...
      {loading ? (
        ...
        <p style={{ margin: 0, fontSize: "14px", lineHeight: "1.85", color: selected ? "#1a3a2a" : "#555" }}>
          {text}
        </p>
      )}
    </div>
...
export default function EndUserScreen() {
  ...
  const [answers, setAnswers] = useState({ rating: 0, highlight: [], feel: "" });
  const [reviews, setReviews] = useState({ casual: "", honest: "", formal: "" });
  const [loadingStates, setLoadingStates] = useState({ casual: false, honest: false, formal: false });
  ...
  const progress = step === "done" ? 100 : step === "generating" ? 90 : ((currentIndex - 1) / 3) * 100;
  ...
  const generateAll = async (currentAnswers) => {
    setStep("generating");
    // 3パターンを並列生成
    const results = await Promise.all(
      STYLES.map(s => generateReview(STORE, currentAnswers, s).catch(() =>
        `${STORE.name}はとても素晴らしいお店でした。${currentAnswers.highlight.join("や")}が特に良く、${currentAnswers.feel}という気持ちです。またぜひ訪れたいと思います。`
      ))
    );
    const newReviews = {};
    STYLES.forEach((s, i) => { newReviews[s.key] = results[i]; });
  ...
  const handlePost = () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      setCopied(true);
      setTimeout(() => window.open(`https://search.google.com/local/writereview?placeid=${STORE.placeId}`, "_blank"), 500);
    });
  };
        ...
        <div style={{ flex: 1, padding: "28px 20px 32px", display: "flex", flexDirection: "column" }}>
          ...
          {step === "welcome" && (
            <div style={{ animation: "fadeUp 0.4s ease", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
              ...
              <p style={{ color: "#888", fontSize: "14px", lineHeight: 1.8, margin: "0 0 32px" }}>3つの質問に答えるだけで<br />AIが口コミ文を自動で作ります</p>
                ...
                {QUESTIONS[1].options.map(opt => {
                  ...
                  return (
                    <button key={opt} onClick={() => setAnswers({ ...answers, highlight: sel ? answers.highlight.filter(v => v !== opt) : [...answers.highlight, opt] })}
                      style={{ padding: "15px 18px", borderRadius: "12px", border: `2px solid ${sel ? "#2C7A4B" : "#E5E7EB"}`, background: sel ? "#F0FAF4" : "#fff", color: sel ? "#1a3a2a" : "#555", fontFamily: "inherit", fontSize: "15px", fontWeight: sel ? "700" : "400", cursor: "pointer", transition: "all 0.18s", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {QUESTIONS[2].options.map(opt => {
                  ...
                  return (
                    <button key={opt} onClick={() => setAnswers({ ...answers, feel: opt })}
                      style={{ padding: "20px 10px", borderRadius: "14px", border: `2px solid ${sel ? "#2C7A4B" : "#E5E7EB"}`, background: sel ? "#2C7A4B" : "#fff", color: sel ? "#fff" : "#555", fontFamily: "inherit", fontSize: "14px", fontWeight: "700", cursor: "pointer", transition: "all 0.18s", textAlign: "center" }}>
                      {opt}
                    </button>
                  );
                })}
          ...
          {step === "done" && (
            <div style={{ animation: "fadeUp 0.4s ease" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: "20px" }}>
                ...
                <h2 style={{ fontSize: "18px", fontWeight: "900", color: "#1a2533", margin: "6px 0 4px" }}>
                  3パターン生成完了！
                </h2>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "16px" }}>
                {STYLES.map(s => (
                  <ReviewCard key={s.key} style={s} text={reviews[s.key]}
                    selected={selectedStyle === s.key} loading={loadingStates[s.key]}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginBottom: "16px" }}>
                ...
                {regenCount > 0 && (
                  <span style={{ fontSize: "11px", color: "#aaa" }}>（{regenCount}回再生成済み）</span>
                )}
              </div>
              ...
              <button onClick={handlePost} disabled={Object.values(loadingStates).some(Boolean)}
                style={{ width: "100%", padding: "18px", borderRadius: "16px", border: "none", background: copied ? "#1A5C38" : "linear-gradient(135deg, #2C7A4B, #3DA66A)", color: "#fff", fontFamily: "inherit", fontSize: "16px", fontWeight: "700", cursor: "pointer", boxShadow: "0 6px 24px rgba(44,122,75,0.3)", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", transition: "all 0.3s" }}>
          ...
          {["welcome","q1","q2","q3"].includes(step) && (
            <button onClick={handleNext} disabled={!canNext()}
              style={{ width: "100%", padding: "18px", marginTop: "24px", borderRadius: "16px", border: "none", background: canNext() ? "linear-gradient(135deg, #2C7A4B, #3DA66A)" : "#E5E7EB", color: canNext() ? "#fff" : "#aaa", fontFamily: "inherit", fontSize: "16px", fontWeight: "700", cursor: canNext() ? "pointer" : "not-allowed", boxShadow: canNext() ? "0 4px 16px rgba(44,122,75,0.3)" : "none", transition: "all 0.2s" }}>
